...
function MessageBox({ socket, setrecordingDelete, currentConversation, messages, message, setmessage, handleSendMessage, startRecording, imageSrc, setImageSrc, customerNumber, setChatOpen, setcustomerNumbers, setSelectedChat, isSubadmin, customerNumbers }) {
    ...
    const handleTransferClick = async () => {
        ...
        const res = await addMember({ memberId: selectedEmployeeId }, currentConversation._id)
        socket.emit("transfer:employee", { userId: selectedEmployeeId })
        setcustomerNumbers(prev => {
            let copy = JSON.parse(JSON.stringify(prev));
            copy = copy.map(customers => {
                if (customers.customerNumber == customerNumber.customerNumber) {
                    customers.employeeId = res.data.employeeId;
                }
                return customers;
            })
            return copy
        ...
        customerNumbers.forEach((customers, index) => {
            if (customers.customerNumber == customerNumber.customerNumber && !customers.employeeId) {

                if (!(index + 1 >= customerNumbers.length)) {
                   
                    setSelectedChat(customerNumbers[index + 1])
                }else{
                    setSelectedChat({});
                }
            }
        ...
        // console.log("trasnfered",res.data.employeeId)
    }
    return (
        <div>
